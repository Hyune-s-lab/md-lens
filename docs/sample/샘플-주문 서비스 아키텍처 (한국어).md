...
> [!NOTE]
> 알림은 GitHub 스타일 콜아웃으로 렌더링됩니다 — Note, Tip, Important, Warning, Caution 다섯 종류.[^alerts]

> [!WARNING]
> 게이트웨이는 인증되지 않은 호출을 주문 서비스에 도달하기 전에 거부합니다.

[^alerts]: 각주는 문서 끝에 모이며 본문으로 되돌아가는 링크가 붙습니다.
